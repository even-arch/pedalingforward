...
export async function GET(req: Request) {
  ...
  const settings = await writeClient.fetch(
    `*[_type == "siteSettings" && _id == $id][0]{adminPassword, anthropicApiKey, openaiApiKey, firecrawlApiKey, pixabayApiKey, telegramBotToken, telegramChatId, aiWritingRules}`,
    { id: SETTINGS_ID },
    { cache: "no-store" }
  );
...
export async function POST(req: Request) {
  ...
  const { adminPassword, anthropicApiKey, openaiApiKey, firecrawlApiKey, pixabayApiKey, telegramBotToken, telegramChatId, aiWritingRules } = body as Record<string, string>;
  ...
  if (pixabayApiKey !== undefined) patch.pixabayApiKey = pixabayApiKey;
